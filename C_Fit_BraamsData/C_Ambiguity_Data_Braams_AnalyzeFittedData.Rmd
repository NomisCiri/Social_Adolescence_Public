...
ggplot(Argh,aes(y=delta,x=id))+geom_bar(stat="identity",width = 0.7,color="black",fill="grey")+
  scale_x_continuous(name="Models",breaks=c(1,2,3,4,5),labels=Argh$ModelName)+
  ylab(expression(paste(Delta," DIC")))+coord_flip()->B
  print(B)
...
```{r include=FALSE}
# RaincloudTibbleBraams<- RaincloudTibbleBraams%>%dplyr::mutate(
#   Group = case_when(
#     Group=="12-13"~1,# AMBIGUITY IS RECODED AS 0
#     Group=="14-15"~2,# RISK IS RECODED AS 1
#     Group=="16-17"~3,
#     Group=="18-19"~4,
#     Group=="20-21"~5,
#     Group=="22"~6,
...
print(C)
ggsave("../X_Figures/Adv_Braams.pdf",height = 7,width=10)
```
...
```{r,fig.show='hold',fig.align='center',fig.height= 7, fig.width=12}
...
```{r,fig.show='hold',fig.align='center',fig.height= 5, fig.width=12}
...
Bin=as.factor(Age)
  ...
  scale_color_viridis(discrete = TRUE,name="Social\nCond",breaks=c("A","B","F"),labels=c("Risk","Solo","Safe"))+
  ...
  theme(legend.position = c(0.8, 0.23))->D
...
cowplot::plot_grid(A,NULL,B,C,NULL,D, labels = c("A","","B","C","","D"),ncol=3, nrow = 2,rel_widths =c(1,0.1,0.5,1,0.1,0.5),align = "hvlr",axis="1")

ggsave("../X_Figures/BraamsWholePlot.pdf",height = 10,width=21)
ggsave("../X_Figures/BraamsWholePlot.tiff",height = 10,width=21,dpi=300)
