...
//   ξ∼Normal(μξ+ξ′·σξ). which is defined over here.
  ...
  for (i in 1:N) {// subs
      ...
      tau[i] = exp(mu_tau + sigma_tau * tau_p[i]);//forbid the sampler to become smaller than 0.
      ocu[i] = exp(mu_ocu + sigma_ocu * ocu_p[i]);//frobid the sampler to become smaller than 0.
      }//endsubs
    ...
    ocu_p[i] ~ normal(0, 1.0);
      ...
      for (t in 1:Tsubj[i]) {
		...
		if(risk1Ambig0[i, t]==0){// is it an ambiguity trial?
			...
			U_safe  = (0.5-(beta[i]*(ambigLevel[i,t]/2))) * pow(safe_Hpayoff[i, t], rho[i])  + (1-(0.5-(beta[i]*(ambigLevel[i,t]/2)))) * pow(safe_Lpayoff[i, t], rho[i]);
			U_risky = (0.5-(beta[i]*(ambigLevel[i,t]/2))) * pow(risky_Hpayoff[i, t], rho[i]) + (1-(0.5-(beta[i]*(ambigLevel[i,t]/2)))) * pow(risky_Lpayoff[i, t], rho[i]);
		}
