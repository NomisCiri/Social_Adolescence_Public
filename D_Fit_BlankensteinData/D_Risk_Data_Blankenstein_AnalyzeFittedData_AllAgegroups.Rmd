...
You need to change the Directory to ModelComparison, in case you want to look at the results of the Risk ambiguity model. As it is now, only trials that were subject to risk are part of the analyis.
...
for(AgeIDX in 1:nGroups){
  for (ModelIDX in 1:nModels){
    ...
    load(paste0("Risk_Models/Age",Agegroups[AgeIDX],"_Blank_ModelFits",Models[ModelIDX],".RData"))# load early adolescents.
...
for(AgeIDX in 1:nGroups){
  ...
  load(paste0("Risk_Models/Age",Agegroups[AgeIDX],"_Blank_ModelFits",Models[WinnerIDX],".RData"))
  ...
  # THIS IS NOT PERFECTLY AUTOMIZED AND WILL CRASH IF ANOTHER MODEL WINS! Adjust to the actual parameters in the model.
...
```{r ,fig.height = 10, fig.width = 20,fig.show='hold',fig.align='center'}
Matrix<-RaincloudTibbleBlank[c(1,3,5,7)]
...
```{r,fig.show='hold',fig.align='center',eval=FALSE}
...
## A small regression
Here i judge if there is a credible age trend in these parameters.
...
```{r}
GroupedBlanky<-BlankensteinGamble%>%filter(typeRA==1)%>% 
