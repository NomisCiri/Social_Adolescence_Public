...
for (g in 1:G){// groups.
  for (i in 1:N) {// subs
      ...
      ocu[i,g] = mu_ocu[g] + sigma_ocu[g] * ocu_p[i,g];
      }//endsubs
    }//endgroups
...
model {
  ...
  for (g in 1:G){// grouploop.
    mu_rho[g]  ~ normal(0.3,0.1);
    mu_tau[g]  ~ normal(0.1,0.1);
     if (g == 1){
    mu_ocu[g]  ~ normal(-5,0.5);
    } else if (g == 2){
    mu_ocu[g]  ~ normal(5,0.5);
    }
    
    sigma_rho[g] ~ normal(0, 0.1);
    sigma_tau[g] ~ normal(0, 0.1);
    sigma_ocu[g] ~ normal(0, 1.0);
  ...
  for (g in 1:G){
      for (i in 1:N) {
        ...
        for (t in 1:Tsubj[i,g]) {
          ...
          log_lik[i,g] = log_lik[i,g] + bernoulli_logit_lpmf(choice[i, t,g] |(1/tau[i,g]) * (U_risky - U_safe));// i defined it before. i dont want it "choice wise"
