  ...
  // I define the distributions in the loop bc of my nested data i have too many dimensions for vectorizing.
    ...
    ocu_p[i] ~ normal(0, 1.0);
      ...
      for (t in 1:Tsubj[i]) {
        ...
        choice[i, t, g] ~ bernoulli((1-ocu[i,g])* inv_logit((1/(tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
        }
        if (condition[i, t , g] == 3) {  // risky-risky
        choice[i, t, g] ~ bernoulli((1-ocu[i,g])* inv_logit((1/(tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
        } else {
        choice[i, t, g] ~ bernoulli_logit((1/tau[i,g]) * (U_risky - U_safe));
        }
      }
    }
}
...
generated quantities {
 ...
 real mu_ocuPred;
  ...
  { // local section, this saves time and space
      ...
      if (condition[i, t,g] == 1) {  // safe-safe
          log_lik[i,t,g] = bernoulli_lpmf(choice[i,t,g] |  ((1-ocu[i,g])* inv_logit((1/(tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) ));// i defined it before. i dont want it "choice wise"
          y_pred[i, t,g] = bernoulli_rng((1-ocu[i,g])*inv_logit((1/(tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
          }
          if (condition[i, t,g] == 3) {  // risky-risky
          log_lik[i,t,g] = bernoulli_lpmf(choice[i,t,g] |  ((1-ocu[i,g])* inv_logit((1/(tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) ));// i defined it before. i dont want it "choice wise"
          y_pred[i, t,g] = bernoulli_rng((1-ocu[i,g])*inv_logit((1/(tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
          } else{
          log_lik[i,t,g] = bernoulli_logit_lpmf(choice[i, t,g] | (1/tau[i,g]) * (U_risky - U_safe));// i defined it before. i dont want it "choice wise"
          // but i produce "pointwise aggregates"
          // generate posterior prediction for current trial
          y_pred[i, t,g] = bernoulli_rng(inv_logit((1/tau[i,g]) * (U_risky - U_safe)));
        }//endifelse
        }//end Trials
      }//end Subs
  }//end local
