...
for (g in 1:G){// groups.
  for (i in 1:N) {// subs
      ...
      ocu[i,g] = Phi_approx(mu_ocu[g] + sigma_ocu[g] * ocu_p[i,g]);
      }//endsubs
    }//endgroups
  ...
  // Fuck Vercorizing I define the distributions in the loop.
    ...
    ocu_p[i,g] ~ normal(0, 1.0);
      ...
      for (t in 1:Tsubj[i,g]) {
        ...
        choice[i, t, g] ~ bernoulli((1-ocu[i,g])* inv_logit(((tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
        }
...
generated quantities {
 ...
 real mu_ocuPred[G];
  ...
  mu_ocuPred[g] = Phi_approx(mu_ocu[g]);
  }
  ...
  for (g in 1:G){
      for (i in 1:N) {
        ...
        for (t in 1:Tsubj[i,g]) {
          ...
          if (condition[i, t,g] == 1) {  // safe-safe
          log_lik[i,t,g] = bernoulli_lpmf(choice[i,t,g] |  ((1-ocu[i,g])* inv_logit(((tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) ));// i defined it before. i dont want it "choice wise"
          y_pred[i, t,g] = bernoulli_rng((1-ocu[i,g])*inv_logit(((tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
          }
          if (condition[i, t,g] == 3) {  // risky-risky
          log_lik[i,t,g] = bernoulli_lpmf(choice[i,t,g] |  ((1-ocu[i,g])* inv_logit(((tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) ));// i defined it before. i dont want it "choice wise"
          y_pred[i, t,g] = bernoulli_rng((1-ocu[i,g])*inv_logit(((tau[i,g]) * (U_risky - U_safe))) +(ocu[i,g]/2) );
