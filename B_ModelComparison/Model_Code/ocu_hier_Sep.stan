...
parameters {
    ...
    real<lower=0> mu_ocuSafe[G];
    real<lower=0> mu_ocuRisk[G];
    
    //roup Sigmas
    real<lower=0>sigma_rho[G];
    real<lower=0>sigma_tau[G];
    real<lower=0>sigma_ocuSafe[G];
    real<lower=0>sigma_ocuRisk[G];
  ...
  transformed parameters {
   ...
   real<lower=0>   ocuSafe[N,G];
   real<lower=0>   ocuRisk[N,G];
  ...
  for (g in 1:G){// groups.
    for (i in 1:N) {// subs
        rho[i,g] = Phi_approx(mu_rho[g] + sigma_rho[g] * rho_p[i,g]) * 2; // i dont quite understand this part. but it makes the correct estimates. Good i would say.
        tau[i,g] = exp(mu_tau[g] + sigma_tau[g] * tau_p[i,g]);
        ocuSafe[i,g] = exp(mu_ocuSafe[g] + sigma_ocuSafe[g] * ocu_pSafe[i,g]);
        ocuRisk[i,g] = exp(mu_ocuRisk[g] + sigma_ocuRisk[g] * ocu_pRisk[i,g]);
        }//endsubs
      }//endgroups
...
model {
  ...
  for (g in 1:G){// grouploop.
      mu_rho[g]  ~ normal(0,1);
      mu_tau[g]  ~ normal(0,1);
    mu_ocuSafe[g]  ~ normal(0,1);
    mu_ocuRisk[g]  ~ normal(0,1);
    
    sigma_rho[g] ~ normal(0, 0.2);
    sigma_tau[g] ~ normal(0, 0.2);
    sigma_ocuSafe[g] ~ cauchy(0, 1.0);
    sigma_ocuRisk[g] ~ cauchy(0, 1.0);
  ...
  // I define the distributions in the loop bc of my nested data i have too many dimensions for vectorizing.
    ...
    ocu_pSafe[i,g] ~ normal(0, 1.0);
    ocu_pRisk[i,g] ~ normal(0, 1.0);
      ...
      for (t in 1:Tsubj[i,g]) {
        ...
        if (condition[i, t, g] == 1) {  // safe-safe
          U_safe = U_safe + ocuSafe[i,g];
        }
        if (condition[i, t , g] == 3) {  // risky-risky
          U_risky = U_risky + ocuRisk[i,g];
        }
...
generated quantities {
 ...
 real<lower=0> mu_ocuPredSafe[G];
 real<lower=0> mu_ocuPredRisk[G];
  ...
  for (g in 1:G){for i in `seq 7211698 7211722`; do qdel i; done
  mu_rhoPred[g] = Phi_approx(mu_rho[g]) * 2;
  mu_tauPred[g] = exp(mu_tau[g]);
  mu_ocuPredSafe[g] = exp(mu_ocuSafe[g]);
  mu_ocuPredRisk[g] = exp(mu_ocuRisk[g]);
  }
